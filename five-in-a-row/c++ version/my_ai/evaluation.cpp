...
int evaluate(int player)
{
	...
	//对手冲四活三
	if (m_nRecordCount[opponentPlayer][SFOUR] && m_nRecordCount[opponentPlayer][THREE])
	{
		return -5000;
	}
	...
	{
		//pipeOut("DEBUG %d发现活三", player);
		return m_nRecordCount[player][THREE] > 1 ? 4000 : 3000;
	}

	if (m_nRecordCount[opponentPlayer][THREE] > 1)
	{
		//pipeOut("DEBUG %d发现活三", opponentPlayer);
		return  -2001;
	}
	...
	{
		Opponent_Turn_Value = m_nRecordCount[opponentPlayer][SFOUR] * 15;
	}
	if (m_nRecordCount[opponentPlayer][THREE] == 1)
	{
		Opponent_Turn_Value = m_nRecordCount[opponentPlayer][THREE] * 15;
	}
